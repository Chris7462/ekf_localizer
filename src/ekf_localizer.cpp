...
void EkfLocalizer::run_ekf()
{
  ...
  geometry_msgs::msg::TransformStamped map_base_tf;
  map_base_tf.header.stamp = rclcpp::Node::now();
  map_base_tf.header.frame_id = "map";
  map_base_tf.child_frame_id = "ekf_link";
  map_base_tf.transform = tf2::toMsg(map_base_link_trans);

  // send the transformation
  tf_broadcaster_->sendTransform(map_base_tf);
}
